...
//
//        fetchFiveDayForecast("Albany,OR,US", "imperial")

        /*viewModel.forecastResults.observe(this) { forecastResults ->
            if (forecastResults != null) {
                forecastAdapter.updateForecast(forecastResults)
                forecastListRV.visibility = View.VISIBLE
                forecastListRV.scrollToPosition(0)
                supportActionBar?.title = forecastResults.city.name
            }
        }
    ...
    override fun onResume() {
        ...
        val setCity = sharedPrefs.getString(getString(R.string.pref_city_key), "Albany,OR,US")
        val setUnit = sharedPrefs.getString(getString(R.string.pref_units_key), null)
        viewModel.loadForecastResults(setCity, setUnit, OPENWEATHER_APPID)
    }
