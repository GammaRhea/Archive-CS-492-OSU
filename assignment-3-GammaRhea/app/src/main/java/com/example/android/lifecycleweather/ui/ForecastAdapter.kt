...
class ForecastAdapter(private val onClick: (ForecastPeriod) -> Unit)
    ...
    class ViewHolder(itemView: View, val onClick: (ForecastPeriod) -> Unit)
        ...
        fun bind(forecastPeriod: ForecastPeriod, forecastCity: ForecastCity?) {
            ...
            val setUnits = sharedPrefs.getString(ctx.resources.getString(R.string.pref_units_key), null)
            var unitString: String = when (setUnits) {
                ctx.resources.getString(R.string.pref_units_value_fahrenheit) -> {
                    "F"
                }
                ctx.resources.getString(R.string.pref_units_value_celsius) -> {
                    "C"
                }
                else -> {
                    "K"
            ...
            highTempTV.text = ctx.getString(R.string.forecast_temp, forecastPeriod.highTemp, unitString)
            lowTempTV.text = ctx.getString(R.string.forecast_temp, forecastPeriod.lowTemp, unitString)
